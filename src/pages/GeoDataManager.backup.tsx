...
import { Loader2, Map, Eye, EyeOff, RefreshCw, Filter } from 'lucide-react';
  ...
  const validateLayerById = useCallback(async (layerId: string, layerKey: string) => {
    try {
      ...
      try {
        ...
        setLayerStats(prev => new Map(prev).set(layerKey, { featureCount, bounds: bbox }));
      } catch {
        setLayerStats(prev => new Map(prev).set(layerKey, { featureCount }));
      }
