...
export const MapSearch = ({ onSelect, onClose }: MapSearchProps) => {
  ...
  return (
    <motion.div 
      ...
      className="glass-floating shadow-2xl p-4 w-full max-w-md rounded-2xl border border-white/20"
    >
      <div className="flex items-center gap-2 mb-3">
        <div className="relative flex-1">
          ...
          <Input
            ...
            className="pl-9 pr-9 h-10 glass-base border-white/5 rounded-xl"
            autoFocus
          />
          {loading && (
        ...
        <div className="space-y-2">
          {[1, 2, 3].map((i) => (
            <motion.div 
              ...
              className="flex items-start gap-3 px-4 py-3 rounded-xl glass-base animate-pulse"
            >
          ...
          >
            {results.map((r, i) => (
              <motion.button
                ...
                className="w-full text-left px-4 py-3 rounded-xl hover:glass-surface transition-all flex items-start gap-3 border border-transparent hover:border-white/10 group"
              >
                <div className="w-8 h-8 rounded-full glass-base flex items-center justify-center shrink-0 group-hover:bg-primary/20 transition-colors">
