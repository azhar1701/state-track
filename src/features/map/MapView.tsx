          ...
          if (!fc && key === 'assets') {
            try {
              ...
              if (!assetError && assetRows) {
                ...
                logger.warn('Failed to fetch assets fallback layer', sanitizeForLog(assetError));
              }
            } catch (assetFallbackError) {
              logger.warn('Failed to build assets feature collection', sanitizeForLog(assetFallbackError));
            }
          }
 ...
 `;
  ...
  useEffect(() => {
    ...
    try {
      ...
      logger.warn('Failed to load cluster radius', sanitizeForLog(e));
    }
  ...
  useEffect(() => {
    ...
    try {
      ...
      logger.warn('Failed to load heatmap radius', sanitizeForLog(e));
    }
        ...
        <div className={`relative rounded-lg overflow-hidden shadow-lg border ${isMobile ? 'h-[calc(100dvh-120px)]' : 'h-[calc(100vh-180px)]'} ${activeMapTool ? 'cursor-crosshair' : ''}`}>
          ...
          {routingPath && (
            <div className="absolute z-[1200] top-40 right-4">
              <Button
                ...
                className="bg-popover/95 border-border shadow-lg shadow-lg font-bold"
              >
                Hapus Rute
              </Button>
            </div>
          ...
          {(loading || adminLoading || Object.entries(dynamicLoading).some(([k, v]) => overlays.dynamic?.[k] && v)) && (
            <div className="absolute top-20 left-1/2 -translate-x-1/2 z-[1100] flex flex-col gap-2 pointer-events-none">
              {loading && (
                <div className="bg-slate-900/90 border border-slate-700 rounded-lg px-4 py-2 shadow-md">
