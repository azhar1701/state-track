...
export const FilterPanel = ({ filters, onFilterChange, onClose }: FilterPanelProps) => {
  ...
  return (
    <motion.div 
      ...
      className="fixed inset-y-0 right-0 z-[1200] w-85 glass-overlay shadow-2xl pointer-events-auto overflow-y-auto border-l border-white/10 flex flex-col"
    >
      <div className="sticky top-0 glass-surface backdrop-blur-xl border-b border-white/10 px-6 py-4 flex items-center justify-between z-10">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl glass-floating flex items-center justify-center text-primary">
        ...
        {onClose && (
          <Button variant="ghost" size="icon" onClick={onClose} className="rounded-xl hover:glass-floating">
            <X className="w-5 h-5" />
          </Button>
        )}
      </div>
      ...
      <div className="p-6 space-y-8 flex-1">
        {user && (
          <div className="space-y-4">
            ...
            <div className="glass-surface rounded-2xl p-2 border border-white/5">
              {loadingPresets ? (
                    ...
                    {presets.map((p, idx) => (
                      ...
                      >
                        <Button 
                          ...
                          className="flex-1 text-left justify-start h-10 px-3 rounded-xl hover:glass-floating border border-transparent hover:border-white/10 transition-all"
                        >
                        ...
                        <Button 
                          ...
                          className="h-9 w-9 rounded-xl text-muted-foreground hover:text-destructive hover:glass-base transition-colors opacity-0 group-hover:opacity-100"
                        >
                          <X className="w-4 h-4" />
                        </Button>
                      </motion.div>
            ...
            <div className="flex items-center gap-2 pt-2">
              <Input 
                ...
                className="h-10 glass-base border-white/5 rounded-xl focus:ring-primary/20"
              />
        ...
        <div className="space-y-6">
          <div className="space-y-3">
            ...
            <div className="space-y-4 glass-surface rounded-2xl p-5 border border-white/5">
              <div className="space-y-2.5">
                ...
                >
                  <SelectTrigger className="h-11 glass-base border-white/5 rounded-xl">
                    <SelectValue placeholder="Semua kategori" />
                  </SelectTrigger>
                  <SelectContent className="glass-overlay border-white/10 rounded-xl">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3 glass-surface rounded-2xl p-5 border border-white/5">
              <div className="space-y-2.5">
                ...
                <Input
                  ...
                  className="h-10 glass-base border-white/5 rounded-xl text-xs"
                />
              </div>
              <div className="space-y-2.5">
      ...
      <div className="sticky bottom-0 glass-surface border-t border-white/10 p-6 flex gap-3 backdrop-blur-xl z-10">
        <Button 
          variant="outline" 
          onClick={handleReset} 
          className="flex-1 h-12 glass-base border-white/10 rounded-xl hover:bg-white/5 transition-all"
        >
          Reset
        </Button>
        <Button 
          onClick={handleApply} 
          className="flex-1 h-12 bg-primary hover:bg-primary/90 rounded-xl shadow-lg shadow-primary/20 transition-all font-bold"
        >
          Terapkan
        </Button>
      </div>
